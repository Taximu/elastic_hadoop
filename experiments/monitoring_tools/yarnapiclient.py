...
def getcontainersPerNM(vmip):
    containersPerNM = 0
    rm = ResourceManager(resourcemanagerhost)
    nodes = rm.cluster_nodes()
    nodesdata = nodes.data['nodes']['node']
    for node in nodesdata:
        nodeHttpAddress = node['nodeHTTPAddress']
        numContainers = node['numContainers']
        if vmip.strip() in nodeHttpAddress:
            containersPerNM = numContainers
            break
    return containersPerNM   
