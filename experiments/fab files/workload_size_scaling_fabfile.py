...
mapred = {
     'wordcount': ['16', '48', '16', '48', 'DATASIZE=100000000', 'DATASIZE=100000000'],
     'sort':      ['16', '48', '16', '48', 'DATASIZE=100000000', 'DATASIZE=100000000'],
     'pagerank':  ['96', '48', '96', '48', 'PAGES=500000', 'PAGES=1000000']
}

benchmarks = ['wordcount', 'sort', 'pagerank']

path = '/home/gorbunovmaksim/Documents/Elastic Hadoop/experiments/hadoop_configs/HORIZONTAL_SCALING/'
...
@roles('master')
def configbench(bench, oldmaps, oldreducers, newmaps, newreducers, olddatasize, newdatasize):
    sed('/mnt/maxim/HiBench/' + bench + '/conf/configure.sh', 'NUM_MAPS=' + oldmaps, 'NUM_MAPS=' + newmaps, use_sudo = False)
    sed('/mnt/maxim/HiBench/' + bench + '/conf/configure.sh', 'NUM_REDS=' + oldreducers, 'NUM_REDS=' + newreducers, use_sudo = False)
    sed('/mnt/maxim/HiBench/' + bench + '/conf/configure.sh', olddatasize, newdatasize, use_sudo = False)
    ...
    with settings(warn_only = True):
        ...
        for bench in benchmarks:
            ...
            for expm in sorted(experiments.keys()):
                if bench == 'sort' or bench == 'wordcount':
                    step += step_ws
                    value = 'DATASIZE=' + str(step)
                else:
                    step += step_pr
                    value = 'PAGES=' + str(step)
                execute(configbench, bench, mapred[bench][0], mapred[bench][1], mapred[bench][2], mapred[bench][3], mapred[bench][4], value)
                print '\n@@@@@@@@[' + expm + ']@@@@@@@@'
                print '=========================================================\n'
                ...
                print '=========================================================\n'
                time.sleep(10)
                execute(configbench, bench, mapred[bench][2], mapred[bench][3], mapred[bench][0], mapred[bench][1], value, mapred[bench][4])
