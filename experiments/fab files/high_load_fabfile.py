...
configpath = '/home/gorbunovmaksim/Documents/Elastic Hadoop/experiments/monitoring_tools'
...
experiments = {
     'expm1':     ['maxim@stream2', 'maxim@stream4', 'maxim@stream7', 'maxim@stream8', 'maxim@stream9', 'maxim@stream10']
}

path = '/home/gorbunovmaksim/Documents/Elastic Hadoop/experiments/hadoop_configs/HIGH_LOAD/'
...
def runexpm():
    with settings(warn_only = True):
	    ...
	    execute(putconfigs, expm)
            ...
            execute(runbench, 'wordcount')
            execute(runbench, 'sort')
            execute(stopmonitor, expm)
            execute(dumpStats, dumpfile)
            execute(copydumpfiles, expm, 'all')
            execute(clearredis)
            execute(clearlogs)
...
def runbench(bench):
	...
	if bench == 'wordcount':
	  run("screen -d -m ./" + bench + "/bin/run.sh", pty = False)
	else:
