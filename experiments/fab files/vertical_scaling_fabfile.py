...
configpath = '/home/gorbunovmaksim/Documents/ElasticHadoop/Experiments/monitoring/elastic-hadoop/monitoring_tools'
...
env.roledefs = {
    ...
    'workers':     ['maxim@stream6', 'maxim@stream7', 'maxim@stream8', 'maxim@stream9', 'maxim@stream10'],
...
path = '/home/gorbunovmaksim/Documents/Elastic Hadoop/experiments/hadoop_configs/VERTICAL_SCALING/'
...
def copydumpfiles(folder, expm, bench):
    if env.host_string in env.roledefs['logmachine']:
        run('cp /mnt/maxim/stats ' + '/home/maxim/expms/' + folder + '.' + expm + '.' + bench + '.' + 'stats')
    elif env.host_string in env.roledefs['master']: 
        run('python '+ '/tmp/' +'yarnapiclient.py --mode dump --dumpfile /mnt/maxim/exps/' + folder + '.' + expm + '.' + bench + '.stat.perNM')
...
def runexpm():
    with settings(warn_only = True):
		...
		for folder in folders:
                    print '\n@@@@@@@@[' + bench + ']@@@@@@@@'
                    print '=========================================================\n'
                    execute(putconfigs, expm, folder)
                    ...
                    execute(copydumpfiles, folder, expm, bench)
                    execute(clearredis)
                    execute(clearlogs)
...
def startworkers(expm):
    if isSlaveInExpm(expm):
        if env.host_string not in env.roledefs['datanodes']:
